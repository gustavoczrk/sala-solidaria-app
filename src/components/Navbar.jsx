    ...
    <nav className="sticky top-0 z-50 bg-white border-b border-gray-200 py-3 px-4 md:px-8 shadow-sm">
      <div className="container mx-auto flex justify-between items-center">
        ...
        <ul className="hidden md:flex items-center space-x-6">
          ...
          {/* Links Condicionais */}
          {currentUser ? (
            // ... (links de usuário logado - sem alteração)
            <>
              <li>
             ...
             <>
              ...
              {/* BOTÃO CADASTRE-SE (DESKTOP) ATUALIZADO */}
              <li>
                <Link to="/register" className="bg-coral hover:bg-opacity-80 text-white font-semibold py-1 px-3 rounded text-sm transition-colors flex items-center whitespace-nowrap">
          ...
          <ul className="flex flex-col py-4 space-y-4 px-6">
            ...
            {currentUser ? (
              <>
                ...
                <li><button onClick={handleLogout} className="bg-red-500 hover:bg-red-700 text-white font-semibold py-2 px-4 rounded text-sm transition-colors w-full flex items-center justify-center"><FontAwesomeIcon icon={faSignOutAlt} className="mr-2 h-4 w-4" /> Sair</button></li>
              </>
            ) : (
              <>
                ...
                <li>
                  <Link 
                    to="/register" 
                    className="bg-coral hover:bg-opacity-80 text-white font-semibold py-2 px-4 rounded text-sm transition-colors w-full block flex items-center justify-center whitespace-nowrap" 
                    onClick={closeMobileMenu}
                  >
                    <FontAwesomeIcon icon={faUserPlus} className="mr-2 h-4 w-4" /> Cadastre-se
                  </Link>
                </li>
              </>
            )}
